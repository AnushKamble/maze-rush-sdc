...
export const sfx = {
  ...
  eat() {
    ...
    if (nowMs - lastEatAt < 55) return; // avoid a harsh stack of clicks when picking up several runes quickly
    lastEatAt = nowMs;
    const wobble = 820 + Math.random() * 90;
    playTones([{ freq: wobble, duration: 0.045, type: "sine", slideTo: wobble * 1.25, gain: 0.045 }]);
  },
  powerUp() {
    playTones([
